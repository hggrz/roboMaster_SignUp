~(function(win){
	var htmls = '<input type="file" name="" id="" class="GoodImg" style="display: none" accept="image/jpeg,image/jpg,image/png" multiple>'+
				'<div class="header">'+
				'    <span class="imgClick">'+
				'    </span>'+
				...
				'    </span>'+
				'</div>'+
				'<div class="GoodImgAll">'+
				'    <ul>'+
				'    </ul>'+
				'</div>';
	var GoodImgFileUploade = function(obj,fn){
		var _this = this;
	...
	GoodImgFileUploade.prototype = {
		...
		ready : function(){
			...
			this.dom.className = 'GoodImgFileUploade';
			this.dom.innerHTML = htmls;
			this.bom.appendChild(this.dom);
			this.files = this.bom.querySelector('.GoodImg');
			...
			this.imgcontent.innerHTML = '最大上传<b style="color:red">'+this.MAX+'</b>张'+_self.MW+' * '+_self.MH+'像素的图片,单个图片不能大于3M[允许上传图片格式:jpeg/jpg/png]';
		},
		fileBtn : function(c,f){
			var _self = this;
			var _imgAll = $(c).parent().parent().find('.GoodImgAll ul');
			$(c).off().on('click',function(){
				$(f).click();
				$(f).off().on('change',function(){
					var _this = this;
					if(_this.files.length!=0){//选择文件时在进入下一步操作
						$(".GoodImg ul").html("");
	...
	var _private = {
		startUploadImg : function(o,files,MAX,callback,W,H){
			...
			for(var f=0;f<files.length;f++){
				if(files[f].size/1024/1000>3){
					msg('单张图片不能大于3M',2);
					fizebool=false;
					break;
				}
			}
			if(fizebool==false){return false;}
	...
	ImgFileupload.prototype.init =function() {
		...
		this.dom.innerHTML = 
							'    <img name="imgs" src="../static/image/loading.gif" alt="" data-src="'+this.imgSrc +'" class="imsg">'+
							'    <i class="delImg">'+
							'        X'+
							'    </i>';
	...
	win.GoodImgFileUploade = GoodImgFileUploade;
})(window);